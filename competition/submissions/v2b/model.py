...
def predict(input: dict, labeled: list[dict] | None = None) -> float:
    ...
    try:
        ...
        # Get subject prior — use most specific available
        subj_bench_prior = BENCH_SUBJ_MEANS.get((benchmark, subject_content))
        subj_prior = SUBJECT_MEANS.get(subject_content, GLOBAL_MEAN)
        prior = subj_bench_prior if subj_bench_prior is not None else subj_prior

        # Shrink NCF prediction toward prior — lean heavily on known stats
        SHRINK = 0.4  # 40% NCF, 60% prior
        prob = SHRINK * ncf_prob + (1.0 - SHRINK) * prior
