...
def predict(input: dict, labeled: list[dict] | None = None) -> float:
    ...
    try:
        ...
        # Handle unseen benchmarks: skip benchmark bias entirely
        bench_idx = BENCHMARK_TO_IDX.get(benchmark)
        b_t = torch.tensor([bench_idx], dtype=torch.long) if bench_idx is not None else None
        ...
        ncf_prob = 1.0 / (1.0 + math.exp(-logit))

        # Get subject prior (known subjects at test time)
        subj_prior = SUBJECT_MEANS.get(subject_content, GLOBAL_MEAN)

        # Shrink NCF prediction toward subject mean to reduce overconfidence
        # on cold-start items where embeddings may be unreliable
        SHRINK = 0.6  # 60% NCF, 40% subject prior
        prob = SHRINK * ncf_prob + (1.0 - SHRINK) * subj_prior

        # If labeled examples available, do simple calibration shift
        if labeled and len(labeled) >= 2:
            labels = [float(ex.get("label", 0.5)) for ex in labeled]
            obs_mean = np.mean(labels)
            # Shift prediction toward observed label rate
            prob = 0.7 * prob + 0.3 * obs_mean

    except Exception:
        # Hierarchical fallback: bench+subject > subject > benchmark > global
    ...
    # Conservative clipping — avoid catastrophic log-loss on wrong predictions
    return float(max(0.03, min(0.97, prob)))
